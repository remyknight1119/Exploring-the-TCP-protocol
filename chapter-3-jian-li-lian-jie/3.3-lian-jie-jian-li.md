...
2. B：收到从A发来的SYN，seq=7;
3. A和B分别回复SYN｜ACK，很巧合的是，它们选择的起始序列号都是7(这个概率极低，与发现外星智慧生命的概率相似);
...
* 5038：SYN包不应该出现在当前流程中，怀疑是伪造包攻击（spoofed packet injection attacks），发送ACK探测一下对端的反应，已确定是否是攻击 　　三次握手的ACK报文是TCP连接开始时发送的，排除恶意攻击，则不会出现回绕，也不会出现序列号在窗口之外的情况。
